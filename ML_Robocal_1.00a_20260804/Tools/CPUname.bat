...
:CPUname
set /p CPUName=<CPUName.dat

Tools\LANRS-diags.exe /e -ip %ip% -w -ex -f "cmd.exe /c %tool_path%\CPU-diags.exe -nl /name >%log_path%"

rem get log file from DUT
Tools\LANRS-diags.exe /Q -ip %ip% -rf "%log_path%"

tools\pt-diags.exe /find %log% "%CPUName%"
